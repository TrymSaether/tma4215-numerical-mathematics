...
\subsection{Recursive Divide-and-Conquer Structure}
...
\paragraph{Twiddle Factor}

The \textbf{twiddle factor} $e^{-2\pi i n / N}$ is the complex exponential that introduces the necessary phase shift when combining the even and odd DFTs.

\paragraph{Butterfly Operation}

The basic computation in the FFT is the \emph{butterfly operation}, which combines two values (from the even and odd DFTs) using the twiddle factor:
...
\subsection{Riemann Integral on the Circle}

Let \(f \in R(\mathbb{T})\) be a \(2\pi\)-periodic function, and let
\[
    \hat{f}(n)=\frac{1}{2\pi}\int_{-\pi}^{\pi} f(x)e^{-inx}\,dx,
    \qquad n\in\mathbb{Z},
\]
denote its \(n\)-th Fourier coefficient.

We first establish a useful identity for \(\hat{f}(n)\) when \(n\neq 0\).

\begin{proposition}{Difference formula for Fourier coefficients}{difference-formula-fourier}
    For every \(n\neq 0\),
    \begin{equation}
        \hat{f}(n)
        =
        \frac{1}{4\pi}\int_{-\pi}^{\pi}
        \left(f(x)-f\left(x+\frac{\pi}{n}\right)\right)e^{-inx}\,dx.
    \end{equation}
\end{proposition}
...
\begin{proof}
    ...
    \begin{equation}
        \hat{f}(n)=\frac{1}{2\pi}\int_{-\pi}^{\pi} f(x)e^{-inx}\,dx.
    \end{equation}
    Since \(f\) is \(2\pi\)-periodic, the function \(x \mapsto f(x)e^{-inx}\) is also \(2\pi\)-periodic. Therefore, translating the interval of integration by \(\pi/n\) does not change the value of the integral. Hence
    \begin{align*}
        \hat{f}(n)
         & = \frac{1}{2\pi}\int_{-\pi}^{\pi} f\left(x+\frac{\pi}{n}\right)e^{-in\left(x+\frac{\pi}{n}\right)}\,dx \\
         & = \frac{1}{2\pi}\int_{-\pi}^{\pi} f\left(x+\frac{\pi}{n}\right)e^{-inx}e^{-i\pi}\,dx                   \\
         & = -\frac{1}{2\pi}\int_{-\pi}^{\pi} f\left(x+\frac{\pi}{n}\right)e^{-inx}\,dx,
    \end{align*}
    since \(e^{-i\pi}=-1\).

    Adding this identity to the original expression for \(\hat{f}(n)\), we obtain
    \begin{align*}
        2\hat{f}(n)
        ...
        \left(f(x)-f\left(x+\frac{\pi}{n}\right)\right)e^{-inx}\,dx.
    \end{align*}
    Dividing by \(2\) yields the desired formula.
\end{proof}

Next, assume that \(f\) is Hölder continuous with exponent \(\alpha>0\). By definition, this means that there exists a constant \(C>0\) such that
\begin{equation}
...
we obtain
\begin{equation}
    \left|f(x)-f\left(x+\frac{\pi}{n}\right)\right|
    \leq C\left|\frac{\pi}{n}\right|^\alpha.
\end{equation}

We can now derive a decay estimate for the Fourier coefficients.

\begin{proposition}{Decay under Hölder continuity}{holder-decay-fourier}
    If \(f\) is Hölder continuous with exponent \(\alpha>0\), then for every \(n\neq 0\),
    \begin{equation}
        |\hat{f}(n)| \leq \frac{C}{2}\left(\frac{\pi}{|n|}\right)^\alpha.
    \end{equation}
    In particular,
    \begin{equation}
        |\hat{f}(n)| = O\!\left(|n|^{-\alpha}\right)
        \qquad \text{as } |n|\to\infty.
    \end{equation}
\end{proposition}

\begin{proof}
    By Proposition~\ref{prop:difference-formula-fourier},
    \begin{align*}
        |\hat{f}(n)|
         & = \left|
        \frac{1}{4\pi}\int_{-\pi}^{\pi}
        ...
        \left|f(x)-f\left(x+\frac{\pi}{n}\right)\right|\,dx                      \\
         & \leq \frac{1}{4\pi}\int_{-\pi}^{\pi}
        C\left|\frac{\pi}{n}\right|^\alpha\,dx                                   \\
         & = \frac{1}{4\pi}\cdot 2\pi \cdot C\left(\frac{\pi}{|n|}\right)^\alpha \\
         & = \frac{C}{2}\left(\frac{\pi}{|n|}\right)^\alpha.
    \end{align*}
    This proves the estimate.
\end{proof}

\begin{remark}{Lipschitz case}{lipschitz-case-fourier}
    If \(f\) is Lipschitz continuous, then \(\alpha=1\), and thus
    \begin{equation}
